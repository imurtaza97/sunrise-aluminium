...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('/api/ga-data');
        if (!response.ok) {
        ...
        if (result.rows && result.rows.length > 0) {
          ...
          const topCountry = result.rows[0].dimensionValues[0]?.value || '0';
          const topCity = result.rows[0].dimensionValues[1]?.value || '0';
          setData({
            totalUsers,
            totalUsersToday,
            topCountry,
        ...
        // Fetch chart data
        const chartResponse = await fetch('/api/ga-chart-data');
        if (!chartResponse.ok) {
          throw new Error(`HTTP error! status: ${chartResponse.status}`);
        }
        const chartResult = await chartResponse.json();

        // Create a map for quick lookup of active users by month number
        const userData = chartResult.reduce((acc, item) => {
          acc[item.date] = item.activeUsers;
          return acc;
        }, {});

        // Generate labels and data, ensuring all months are included
        const labels = allMonths;
        const dataset = labels.map((month, index) => {
          const monthNumber = String(index + 1).padStart(2, '0'); // Convert index to "01", "02", etc.
          return userData[monthNumber] || 0; // If no data for the month, use 0
        });

        setChartData({
          labels, // Array of month names from January to December
          datasets: [
            {
              label: 'Active Users',
              data: dataset, // Array of active user counts with 0 for missing months
              borderColor: 'black',
